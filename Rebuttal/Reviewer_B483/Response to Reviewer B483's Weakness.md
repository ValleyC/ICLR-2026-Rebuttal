...
**(d)** In Section 4.1, we have documented that curriculum learning is used for large-scale problems. TSP-500/1000 are initialized from the TSP-100 checkpoint, and TSP-10000 is initialized from the TSP-500 checkpoint. This is also a standard approach in competing diffusion-based solvers [2], [3], and [9]. Curriculum learning is used to improve training efficiency and for fair comparisons. But it is not strictly necessary. Direct training on target problem sizes also converges successfully with more training data and longer training time.